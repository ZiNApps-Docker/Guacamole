...
RUN apt install -y build-essential libcairo2-dev libjpeg62-turbo-dev libpng-dev libossp-uuid-dev libavcodec-dev libavutil-dev libswscale-dev libfreerdp-dev libpango1.0-dev libssh2-1-dev libtelnet-dev libvncserver-dev libpulse-dev libssl-dev libvorbis-dev libwebp-dev freerdp2-x11 ghostscript jq wget curl
...
RUN cd /tmp && \
    ...
    wget $SERVER/incubator/guacamole/0.9.13-incubating/binary/guacamole-auth-jdbc-0.9.13-incubating.tar.gz && \
    tar -xzf guacamole-auth-jdbc-0.9.13-incubating.tar.gz && \
    service mysql restart && \
    mysql -u root -e "create database guacamole_db" && \
    mysql -u root -e "create user 'guacamole_user'@'localhost' identified by 'guacamole_pass'" && \
    mysql -u root -e "GRANT SELECT,INSERT,UPDATE,DELETE ON guacamole_db.* TO 'guacamole_user'@'localhost'" && \
    mysql -u root -e "flush privileges" && \
    cat guacamole-auth-jdbc-0.9.13-incubating/mysql/schema/*.sql | mysql -u root guacamole_db
